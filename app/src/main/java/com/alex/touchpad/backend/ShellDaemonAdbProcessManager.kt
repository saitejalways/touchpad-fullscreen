...
data class ShellDaemonProcessResult(
    val exitCode: Int,
    val output: String,
)

private data class ShellDaemonConnectedDevice(
    val serial: String,
    val state: String,
)

class ShellDaemonAdbProcessManager(private val context: Context) {
    ...
    ): ShellDaemonProcessResult {
        val adbBinary = ensureAdbBinary() ?: return ShellDaemonProcessResult(-1, "adb missing")
        return runProcess(
    ...
    private suspend fun listConnectedDevices(adbBinary: File): List<ShellDaemonConnectedDevice> {
    ...
    private suspend fun runProcess(
        args: List<String>,
        timeoutMs: Long,
    ): ShellDaemonProcessResult = withContext(Dispatchers.IO) {
        runCatching {
            ...
            if (!finished) {
                ...
                return@runCatching ShellDaemonProcessResult(
            ...
            ShellDaemonProcessResult(exitCode = -1, output = error.message.orEmpty())
        }
    }
